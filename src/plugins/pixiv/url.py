...
# def Lusheng(N: int = 1, R18: int = 0):
#     msg = ""
#     if 1 <= N <= 10:
#         msg += f"{Bot_NICKNAME}为你准备了{N}张随机{'r18' if R18 else ''}色图。"
#     elif N > 10:
#         N = 1
#         if R18:
...
#     resp = httpx.get(f"https://api.xilusheng.top/nonebot/pixiv/random/?num={N}&r18={R18}&excludeAI=1")
#     if resp.status_code == 200:
#         resp = resp.text
#         resp = ''.join(x for x in resp if x.isprintable())
#         Lolicon_list = json.loads(resp)["data"]
